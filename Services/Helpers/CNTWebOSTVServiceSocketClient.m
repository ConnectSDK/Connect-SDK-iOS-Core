//
//  CNTWebOSTVServiceSocketClient.m
//  Connect SDK
//
...
@implementation CNTWebOSTVServiceSocketClient
{
    int _UID;
...
-(void) helloTv
{
    ...
    CNTServiceCommand *hello = [[CNTServiceCommand alloc] init];
...
-(void) registerWithTv
{
    CNTServiceCommand *reg = [[CNTServiceCommand alloc] init];
    reg.delegate = self;
...
- (void)webSocket:(LGSRWebSocket *)webSocket didFailWithError:(NSError *)error
{
    ...
    {
        CNTServiceCommand *comm = (CNTServiceCommand *)[_activeConnections objectForKey:key];
...
- (void)webSocket:(LGSRWebSocket *)webSocket didReceiveMessage:(id)message
{
    ...
    CNTServiceCommand *connectionCommand = [_activeConnections objectForKey:connectionKey];
    ...
    } else
    {
        ...
        } else if ([type isEqualToString:@"hello"])
        {
            ...
            CNTServiceCommand *comm = _activeConnections[@"hello"];
    ...
    const BOOL leaveConnection = ([connectionCommand isKindOfClass:[CNTServiceSubscription class]] ||
...
- (CNTServiceSubscription *) addSubscribe:(NSURL *)URL payload:(NSDictionary *)payload success:(SuccessBlock)success failure:(FailureBlock)failure
{
    ...
    CNTServiceSubscription *subscription = [_subscribed objectForKey:subscriptionReferenceId];

    if (subscription == nil)
    {
        int callId = [self getNextId];
        subscription = [CNTServiceSubscription subscriptionWithDelegate:self target:URL payload:payload callId:callId];
...
- (int) sendCommand:(CNTServiceCommand *)comm withPayload:(NSDictionary *)payload toURL:(NSURL *)URL
{
    int callId = [self getNextId];
