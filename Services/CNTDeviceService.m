...
@implementation CNTDeviceService
{
...
+ (instancetype) deviceServiceWithClass:(Class)_class serviceConfig:(CNTServiceConfig *)serviceConfig
{
...
- (instancetype) initWithJSONObject:(NSDictionary *)dict
{
    ...
    if (configDictionary)
        config = [CNTServiceConfig serviceConfigWithJSONObject:configDictionary];
    ...
    if (self)
    {
        ...
        if (descriptionDictionary)
            self.serviceDescription = [[CNTServiceDescription alloc] initWithJSONObject:descriptionDictionary];
    }
