...
@implementation CNTServiceDescription
...
{
    return [[CNTServiceDescription alloc] initWithAddress:address UUID:UUID];
}
...
- (id) copy
{
    CNTServiceDescription *serviceDescription = [[CNTServiceDescription alloc] initWithAddress:[self.address copy] UUID:[self.UUID copy]];
...
- (BOOL)isEqualToServiceDescription:(CNTServiceDescription *)service
{
    if (!service) {
        return NO;
    }
...
- (BOOL)isEqual:(id)object
{
    ...
    if (![object isKindOfClass:[CNTServiceDescription class]]) {
        return NO;
    }
