...
@implementation CNTLaunchSession


+ (CNTLaunchSession *) launchSessionForAppId:(NSString *)appId
{
    CNTLaunchSession *launchSession = [[CNTLaunchSession alloc] init];
    launchSession.appId = appId;

    return launchSession;
}

+ (CNTLaunchSession *) launchSessionFromJSONObject:(NSDictionary *)json
{
    return [[CNTLaunchSession alloc] initWithJSONObject:json];
}
...
- (instancetype) initWithJSONObject:(NSDictionary*)json
{
    ...
    if (self) {
        ...
        self.sessionType = (CNTLaunchSessionType) [[json objectForKey:@"sessionType"] integerValue];
