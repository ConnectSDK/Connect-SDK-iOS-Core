...
#import "CNTSSDPDiscoveryProvider_Private.h"
...
/// but it is not (e.g., -[CNTDLNAService setVolume:success:failure:]).
...
/// Tests for the @c CNTDLNAService class.
@interface CNTDLNAServiceTests : XCTestCase

@property (nonatomic, strong) id serviceCommandDelegateMock;
@property (nonatomic, strong) CNTDLNAService *service;

@end

@implementation CNTDLNAServiceTests

- (void)setUp {
    [super setUp];
    self.serviceCommandDelegateMock = OCMProtocolMock(@protocol(CNTServiceCommandDelegate));
    self.service = [CNTDLNAService new];
...
/// Tests that @c CNTDLNAService parses a UPnP error from a sample Xbox response
/// properly.
...
/// Tests that @c CNTDLNAService parses a UPnP error from a sample Sonos response
/// properly.
...
- (void)testAbsoluteAlbumArtURLWithoutHostShouldBeRelativeToServicesCommandURL {
    id serviceDescriptionMock = OCMClassMock([CNTServiceDescription class]);
...
- (void)testDisconnectShouldStopHTTPServer {
    // Arrange
    id httpServerMock = OCMClassMock([CNTDLNAHTTPServer class]);
    ...
    CNTDLNAService *service = OCMPartialMock([CNTDLNAService new]);
...
- (void)testDisconnectShouldUnsubscribeAllSubscriptions {
    // Arrange
    id httpServerMock = OCMClassMock([CNTDLNAHTTPServer class]);
    id reachabilityMock = OCMClassMock([CNTDeviceServiceReachability class]);
        ...
        return ([request.URL.host isEqualToString:kHOST] &&
                [request.HTTPMethod isEqualToString:@"SUBSCRIBE"]);
                        ...
                        withStubResponse:^OHHTTPStubsResponse *(NSURLRequest *request) {
                            ...
                            // response is handled in CNTDLNAService, for it to
                            // save the SID!
...
- (void)callCommandCallbackFromInvocation:(NSInvocation *)invocation
    ...
    __unsafe_unretained CNTServiceCommand *tmp;
    [invocation getArgument:&tmp atIndex:2];
    CNTServiceCommand *command = tmp;
...
- (void)checkPlayMediaShouldCreateProperSetAVTransportURIXMLWithTitle:(NSString *)sampleTitle
    ...
    [self setupSendCommandTestWithName:@"SetAVTransportURI"
                             namespace:kAVTransportNamespace
                           actionBlock:^{
                               CNTMediaInfo *mediaInfo = [[CNTMediaInfo alloc] initWithURL:[NSURL URLWithString:sampleURL]
                               ...
                               [self.service playMediaWithMediaInfo:mediaInfo
                                                         shouldLoop:NO
                                                            success:^(CNTMediaLaunchObject *mediaLanchObject) {
                                                                XCTFail(@"success?");
                                                            } failure:^(NSError *error) {
                                                                XCTFail(@"fail? %@", error);
                                                            }];
...
- (void)checkUpdateControlURLForDevice:(NSString *)device
    ...
    CNTServiceDescription *serviceDescription = [self serviceDescriptionForDevice:device];
...
- (CNTServiceDescription *)serviceDescriptionForDevice:(NSString *)device {
    NSString *filename = [NSString stringWithFormat:@"ssdp_device_description_%@", device];
    NSData *xmlData = [NSData dataWithContentsOfFile:
                       OHPathForFileInBundle([filename stringByAppendingPathExtension:@"xml"], nil)];
    CNTServiceDescription *serviceDescription = [CNTServiceDescription new];
    ...
    CNTSSDPDiscoveryProvider *ssdp = [CNTSSDPDiscoveryProvider new];
