//
//  CNTSSDPDiscoveryProviderTests.m
//  ConnectSDK
//
...
/// Tests for the CNTSSDPDiscoveryProvider class.
/// In some tests, the CNTSSDPSocketListener class is mocked to verify the output
/// data or provide some input data. Also, HTTP requests/responses are mocked
/// to fake device description data.
@interface CNTSSDPDiscoveryProviderTests : XCTestCase

@property (nonatomic, strong) CNTSSDPDiscoveryProvider *provider;

@end

@implementation CNTSSDPDiscoveryProviderTests
...
- (void)setUp {
    ...
    self.provider = [CNTSSDPDiscoveryProvider new];
}
...
- (void)testDelegateDidFindServiceShouldBeCalledAfterReceivingSearchResponse {
    // Arrange
    id delegateMock = OCMProtocolMock(@protocol(CNTDiscoveryProviderDelegate));
...
- (void)testDelegateDidLoseServiceShouldBeCalledAfterReceivingByeByeNotification {
    ...
    __block CNTServiceDescription *foundService;
    OCMStub([delegateMock discoveryProvider:self.provider
        ...
        __unsafe_unretained CNTServiceDescription *tmp;
    ...
    OCMExpect([delegateMock discoveryProvider:self.provider
                               didLoseService:[OCMArg checkWithBlock:^BOOL(CNTServiceDescription *service) {
