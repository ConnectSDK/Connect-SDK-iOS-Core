//
//  CNTSSDPDiscoveryProvider_FilteringTests.m
//  ConnectSDK
//
...
#import "CNTSSDPSocketListener.h"
...
/// Tests for the @c CNTSSDPDiscoveryProvider 's discovery and filtering features.
@interface CNTSSDPDiscoveryProvider_FilteringTests : XCTestCase

@end

@implementation CNTSSDPDiscoveryProvider_FilteringTests

#pragma mark - DLNA/Netcast Services Filtering Tests

/// Tests that the @c CNTSSDPDiscoveryProvider properly parses Sonos' XML device
...
- (void)testShouldFindDLNAService_Sonos {
    [self checkShouldFindDevice:@"sonos"
       withExpectedFriendlyName:@"Office - Sonos PLAY:1 Media Renderer"
        usingDiscoveryProviders:@[[CNTDLNAService class]]];
}
...
/// Tests that the @c CNTSSDPDiscoveryProvider properly parses Xbox's XML device
/// description with DLNA filter only and accepts the service.
...
- (void)testShouldFindDLNAServiceConsideringNetcast_Sonos {
    ...
    [self checkShouldFindDevice:@"sonos"
       withExpectedFriendlyName:@"Office - Sonos PLAY:1 Media Renderer"
        usingDiscoveryProviders:@[[CNTNetcastTVService class], [CNTDLNAService class]]];
}
...
- (void)testShouldFindFireTVWithDIALService {
    [self checkShouldFindDevice:@"firetv"
       withExpectedFriendlyName:@"Fire TV"
        usingDiscoveryProviders:@[[CNTDIALService class]]];
}
...
- (void)testShouldFindRokuWithRokuService {
    [self checkShouldFindDevice:@"roku2"
       withExpectedFriendlyName:@"Roku2"
        usingDiscoveryProviders:@[[CNTRokuService class]]];
}
...
- (void)testShouldFindWebOSWithWebOSService {
    [self checkShouldFindDevice:@"webos_minor_webos"
       withExpectedFriendlyName:@"MR"
        usingDiscoveryProviders:@[[CNTWebOSTVService class]]];
}
...
- (void)checkShouldFindDevice:(NSString *)device
    ...
    // Arrange
    CNTSSDPDiscoveryProvider *provider = [CNTSSDPDiscoveryProvider new];
    ...
    id searchSocketMock = OCMClassMock([CNTSSDPSocketListener class]);
    ...
    id discoveryProviderDelegateMock = OCMProtocolMock(@protocol(CNTDiscoveryProviderDelegate));
    ...
    if (friendlyName) {
        ...
        OCMExpect([discoveryProviderDelegateMock discoveryProvider:[OCMArg isEqual:provider]
                                                    didFindService:[OCMArg checkWithBlock:^BOOL(CNTServiceDescription *service) {
