//
//  CNTDefaultConnectableDeviceStore.m
//  Connect SDK
//
...
@implementation CNTDefaultConnectableDeviceStore
{
    NSMutableDictionary *_activeDevices; // active CNTConnectableDevice objects
    NSMutableDictionary *_storedDevices; // inactive NSDictionary objects containing CNTConnectableDevice information
...
- (void) updateDevice:(CNTConnectableDevice *)device
{
    ...
    // since this is an update, we will serialize CNTConnectableDevice into JSON manually. This way we avoid
...
- (CNTConnectableDevice *) deviceForId:(NSString *)id
{
    ...
    CNTConnectableDevice *foundDevice = [self activeDeviceForUUID:id];
    ...
    if (!foundDevice)
    {
        ...
        if (foundDeviceInfo)
            foundDevice = [[CNTConnectableDevice alloc] initWithJSONObject:foundDeviceInfo];
    }
...
- (CNTServiceConfig *) serviceConfigForUUID:(NSString *)UUID
{
    ...
    CNTServiceConfig *foundConfig = nil;
    ...
    if (device && ![device isKindOfClass:[NSNull class]])
    {
        ...
        if (services && ![services isKindOfClass:[NSNull class]])
        {
            ...
            if (service && ![service isKindOfClass:[NSNull class]])
            {
                ...
                {
                    foundConfig = [CNTServiceConfig serviceConfigWithJSONObject:serviceConfigInfo];
                }
            }
        }
    }
...
- (CNTConnectableDevice *) activeDeviceForUUID:(NSString *)UUID
{
    __block CNTConnectableDevice *foundDevice = nil;
    ...
    if (!foundDevice)
    {
        [_activeDevices enumerateKeysAndObjectsUsingBlock:^(id key, CNTConnectableDevice *device, BOOL *deviceStop)
        {
            [device.services enumerateObjectsUsingBlock:^(CNTDeviceService *service, NSUInteger idx, BOOL *serviceStop)
            {
...
- (NSDictionary *) jsonRepresentationForDevice:(CNTConnectableDevice *)device
{
    if (!device || device.services.count == 0)
        return nil;
