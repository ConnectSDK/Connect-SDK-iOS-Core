...
/*!
 * CNTCapabilityFilter is an object that wraps an NSArray of required capabilities. This CNTCapabilityFilter is used for determining which devices will appear in CNTDiscoveryManager's compatibleDevices array. The contents of a CNTCapabilityFilter's array must be any of the string constants defined in the Capability header files.
 *
 ...
 *
 * [CNTDiscoveryManager capabilityFilters] is an OR filter. a CNTConnectableDevice only needs to satisfy one condition (CNTCapabilityFilter) to pass.
 *
 * ###Examples
...
@code
    ...
    CNTCapabilityFilter *filter =
        [CNTCapabilityFilter filterWithCapabilities:capabilities];

    [[CNTDiscoveryManager sharedManager] setCapabilityFilters:@[filter]];
@endcode
 *
...
@code
    ...
    CNTCapabilityFilter *videoFilter =
        [CNTCapabilityFilter filterWithCapabilities:videoCapabilities];
    CNTCapabilityFilter *imageFilter =
        [CNTCapabilityFilter filterWithCapabilities:imageCapabilities];

    [[CNTDiscoveryManager sharedManager] setCapabilityFilters:@[videoFilter, imageFilter]];
@endcode
 */
@interface CNTCapabilityFilter : NSObject
...
/*!
 * Create a CNTCapabilityFilter with the given array required capabilities.
 *
 * @param capabilities Capabilities to be added to the new filter
 */
+ (CNTCapabilityFilter *)filterWithCapabilities:(NSArray *)capabilities;
