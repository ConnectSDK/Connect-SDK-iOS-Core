...
@interface CNTSSDPDiscoveryProvider () <NSXMLParserDelegate>
{
...
@implementation CNTSSDPDiscoveryProvider
...
- (void)addDeviceFilter:(NSDictionary *)parameters
{
    ...
    _CNT_assert_state(ssdpInfo != nil, @"This device filter does not have ssdp discovery info");
    
    NSString *searchFilter = [ssdpInfo objectForKey:@"filter"];
    _CNT_assert_state(searchFilter != nil, @"The ssdp info for this device filter has no search filter parameter");
...
- (void) sendRequestForFilter:(NSString *)filter userAgentToken:(NSString *)userAgentToken killInactiveDevices:(BOOL)shouldKillInactiveDevices
{
    if (shouldKillInactiveDevices)
    {
        ...
        @synchronized (_foundServices)
        {
            ...
            {
                CNTServiceDescription *service = (CNTServiceDescription *) [_foundServices objectForKey:key];
            ...
            if (refresh)
            {
                ...
                {
                    CNTServiceDescription *service = [_foundServices objectForKey:key];
    ...
    if (!_searchSocket)
    {
		_searchSocket = [[CNTSSDPSocketListener alloc] initWithAddress:kSSDP_multicast_address andPort:0];
    ...
    if (!_multicastSocket)
    {
		_multicastSocket = [[CNTSSDPSocketListener alloc] initWithAddress:kSSDP_multicast_address andPort:kSSDP_port];
...
- (void)socket:(CNTSSDPSocketListener *)aSocket didReceiveData:(NSData *)aData fromAddress:(NSString *)anAddress
{
            ...
            (theUSSNKey.length > 0))
        {
            ...
            if (theUUID && theUUID.length > 0)
            {
                ...
                if ([theHeaderDictionary[@"NTS"] isEqualToString:@"ssdp:byebye"])
                {
                    @synchronized (_foundServices)
                    {
                        CNTServiceDescription *theService = _foundServices[theUUID];
                ...
                } else
                {
                    ...
                    if (location && location.length > 0)
                    {
                        ...
                        CNTServiceDescription *foundService;
                        CNTServiceDescription *helloService;
                        ...
                        {
                            foundService = [[CNTServiceDescription alloc] init];
...
- (void) notifyDelegateOfNewService:(CNTServiceDescription *)service
{
    ...
    [serviceIds enumerateObjectsUsingBlock:^(NSString *serviceId, NSUInteger idx, BOOL *stop) {
        CNTServiceDescription *newService = [service copy];
...
- (void) notifyDelegateOfLostService:(CNTServiceDescription *)service
{
    NSArray *serviceIds = [self serviceIdsForFilter:service.type];
